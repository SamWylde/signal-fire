...
import { applyInstagramAuth, isLoggedIn } from '../src/platforms/instagram/auth.js';
import { INSTAGRAM } from '../src/platforms/instagram/selectors.js';

// ---------------------------------------------------------------------------
// 1. applyInstagramAuth — no inputs
// ---------------------------------------------------------------------------
describe('applyInstagramAuth', () => {
  it('returns ok:false with reason when no input provided', async () => {
    // Context stub — should never be called
    const context = {
      newPage: vi.fn(),
      addCookies: vi.fn(),
    } as unknown as Parameters<typeof applyInstagramAuth>[0];

    const result = await applyInstagramAuth(context, {});
  ...
  it('uses pre-parsed cookies and does NOT read cookiesFile when cookies is provided', async () => {
    ...
    // Warmup page was opened and closed (via applyCookies navigateUrl)
  ...
  it('rejects blank credentials before submitting the login form', async () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.reason).toBe('Instagram credentials require non-empty username and password');
  });
});

describe('isLoggedIn', () => {
  it('navigates to Instagram home before checking login indicators', async () => {
    ...
    expect(goto).toHaveBeenCalledWith(INSTAGRAM.urls.home, {
